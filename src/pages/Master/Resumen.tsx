...
import { ShoppingCart, DollarSign, Building, Users, TrendingUp, Clock } from 'lucide-react';
...
export default function MasterResumen() {
  const { user, profile } = useAuth();
  const { selectedClientId, clients, isLoading: contextLoading } = useClientContext();
  ...
  const loadMasterStats = async () => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      console.debug('master:stats:fetch:start', selectedClientId);

      // Get date 30 days ago
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

      const [ordersResult, branchesResult, usersResult] = await Promise.all([
      ...
      const recentOrders = orders
        ...
        .map(order => ({
          ...
          branch_name: order.client_branches?.name || 'Sin sucursal'
        }));

      const masterStats: MasterStats = {
      ...
      console.debug('master:stats:fetch:ok', masterStats);
      setStats(masterStats);
    } catch (err) {
      console.error('master:stats:fetch:err', err);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Resumen de la Empresa</h1>
        <p className="text-gray-600">Estadísticas y actividad de {selectedClient?.name}</p>
      </div>
